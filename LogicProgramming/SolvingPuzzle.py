...
#to check whose house is left or next to who's house
#def left(q, p, list):
   # return membero((q, p), zip(list, list[1:]))
def left(q, p, list):
    return conde(
        [eq((q, p), (list[0], list[1]))],
        [eq((q, p), (list[1], list[2]))],
        [eq((q, p), (list[2], list[3]))],
        [eq((q, p), (list[3], list[4]))]
    )
def next(q, p, list):
...
#running the solver with preceding constraints
solutions = run(0, houses, rules_zebraproblem)

#extracting the output from the solver
